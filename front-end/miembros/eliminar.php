    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
          <h1 class="mt-4 fw-bold fs-2 text-dark"><i class="fa-solid fa-user-pen"></i>&nbsp;<font face="helvetica">ELIMINAR MIEMBRO</font></h1>
          <hr>
          <?php
          
          if(isset($_GET["iddel"])){ //Verificar si el usuario quiere eliminar, mandar a formulario de confirmacion
            include_once("formularios/formConfirmarEliminacion.php");
          } 
          
          else if(isset($_GET["msg"])){ // VErificar si ya se elimino el registro, mandar pagina de exito o error
            if($_GET["msg"] == "1"){
              include_once("errors/eliminacionExitosa.php");
            }else {
              include_once("errors/eliminacionFallida.php");
            }
          } 
          
          else if(isset($_GET["confirmDelete"])){ // Verificar si el usuario confirma la eliminacion
            // Incluir las funciones de las consultas hacia la API
            include_once("ConsultasAPI/APIMiembros.php");
            // Ejecutar las consulta de eliminar a la API
            $exito = APIMiembros::eliminarMiembro($_GET["idMiembro"]);
            if($exito && $exito != "error"){ //Si se eliminno
              header("Location: eliminar.php?msg=1");
            }else { // Si no se elimino
              header("Location: eliminar.php?msg=0");
            }
          }
          ?>
        </main>
